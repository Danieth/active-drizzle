...
| 2 | `packages/react/src/hooks.ts` | DORMANT — investigated 2026-07-24, retirement supported | ✅ banner | Phase-4 hook factories (`createModelHook`/`createSearchHook`). Investigation findings: (a) presenters are doctrinally forbidden from fetching — extra data arrives via `props.ctx` (folder `context.ts` client lane + `@frontendContext` server lane), not hooks; (b) pickers read a call-site-chosen controller's GENERATED hooks (`props={{ from: Controller }}`); (c) react-generator emits index/get/mutations/bulk/attachments/@action hooks directly against TanStack Query with coherence invalidation baked in — zero references to these factories anywhere (generator, docs, scaffold); (d) hand-wiring via factories would BYPASS the invalidation graph — strictly worse, not lightweight. No current job, no future job under the current design. Recommend: drop the `index.ts` re-exports at the next minor bump, then delete. Awaiting Daniel's call. |
