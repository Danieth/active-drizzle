...
(* properties.  The split across the two modules: THIS module discharges   *)
(* no-cursor-skip (plus convergence, monotone cursor, and received-        *)
(* soundness as doc-lane shadows of T2(i)/T4/T5).  The other five O8       *)
(* checks -- no-regression, no-resurrection (incl. recreation), "304       *)
(* never freshens a cell the client does not hold," "304 never certifies   *)
(* across a lifecycle event," and "no pre-epoch frame is ever accepted"    *)
(* -- live in specs/RowLane.tla (the row-lane module: Rule M cells/floor,  *)
(* A2' validation, three-phase pulls, T9/O16 epochs), checked under TLC    *)
(* -- see its MODEL-CHECKING RESULTS block.  Doors/epochs (A3, T9, O16)    *)
(* apply to ALL frames, doc frames included; omitting them HERE is sound   *)
(* only because a client's discard of a pre-epoch doc frame is             *)
(* behaviorally identical to Drop (Cw loss), which this model already      *)
(* covers; the epoch filter itself is checked in RowLane                   *)
(* (NoPreEpochAccept), where the channel layer is explicit.                *)
(***************************************************************************)
EXTENDS Naturals
...
(* a liveness counterexample (PerSeqReceipt or ConvergenceAfter-           *)
(* Quiescence) -- any infinite behavior that idles while some Pull(c)      *)
(* stays enabled violates the WF conjunct and is excluded from Spec.       *)
(***************************************************************************)
Deliver ==
  \E m \in chan :
    LET c == m[1]
...
(*   PerSeqReceipt (for n = 3, 4) and ConvergenceAfterQuiescence are both  *)
(*   violated (a lasso: the system quiesces short of                       *)
...
(* Property (2) -- per-seq eventual receipt (T5 + A6, the form T5          *)
(* actually claims): every committed seq eventually reaches every          *)
(* fairly-pulling client.  This does NOT lean on append quiescence: the    *)
(* leads-to must be discharged for each n on every fair behavior,          *)
(* however the ledger keeps growing afterwards.  Why WF(Pull) suffices:    *)
(* while n \in 1..ledgerLen is missing from received[c], cursor[c] < n     *)
(* (the cursor is the contiguous prefix), so Pull(c) is continuously       *)
(* enabled and its range (cursor, ledgerLen] contains n; WF forces the     *)
(* pull, and set-union (A5) never un-receives.  By A5/SEC, equal           *)
(* received sets imply equal Loro documents.                               *)
PerSeqReceipt ==
  \A n \in Seqs :
    (n <= ledgerLen) ~> (\A c \in Clients : n \in received[c])

(* Property (2') -- convergence after quiescence (the whole-set form,      *)
(* named honestly): eventually, permanently, every client's received set   *)
(* equals the committed ledger 1..ledgerLen.  The <>[] target is only      *)
(* reachable because MaxSeq bounds appends -- after the last append, fair  *)
(* pulls close every gap and nothing reopens one.  Under unbounded         *)
(* continuous writes this property would be meaningless (the target set    *)
(* moves forever); PerSeqReceipt above is the statement that survives      *)
(* that limit, and is the one T5 claims.  Kept as a corollary check: it    *)
(* additionally pins the QUIESCENT state (nothing over-received either,    *)
(* via ReceivedSound).                                                     *)
ConvergenceAfterQuiescence ==
  <>[] (\A c \in Clients : received[c] = 1..ledgerLen)

-----------------------------------------------------------------------------
(***************************************************************************)
(* WORKED TRACES (hand-walked illustrations of the definitions; kept as    *)
(* documentation).  TLC, not these, is the evidence: run specs/run-tlc.sh  *)
(* DocLane (TLC from specs/tla2tools.jar) -- with Clients = {c1, c2},      *)
(* MaxSeq = 5 it exhausts the state space (1,118,481 distinct states,      *)
(* depth 16) with every invariant and all three temporal properties        *)
(* (MonotoneCursor, PerSeqReceipt, ConvergenceAfterQuiescence)             *)
(* satisfied; see the run record in DocLane.cfg.  The traces below are     *)
(* worked examples of the two load-bearing scenarios.                      *)
(*                                                                         *)
...
(*     same way; every committed n has landed at every client              *)
(*     (PerSeqReceipt) and ConvergenceAfterQuiescence's target is          *)
(*     reached and (received only                                          *)
...
(*  PerSeqReceipt / ConvergenceAfterQuiescence lasso.  Permanent gap,      *)
(*  exactly landmine 2.                                                    *)
(*                                                                         *)
