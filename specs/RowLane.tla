...
(* frontier.                                                              *)
(*                                                                         *)
(* DEEP GUARDED RUN (same three-phase-pull spec, TLC 2.19, 14 workers,    *)
(* Apple Silicon, 2026-08-27, shipped constants MaxToken = 3,             *)
(* MaxEpoch = 3): NO violation of any invariant or action property in    *)
(* 316,648,704 distinct states (1,795,307,710 generated), BFS frontier   *)
(* at depth 15 -- every state reachable in <= 14 steps checked, vs the   *)
(* pre-fix violation at depth 10.  Stopped by a disk guard at 37 min     *)
(* with ~228M states queued and the frontier still growing: full         *)
(* exhaustion at these constants exceeds this machine's disk (the BFS    *)
(* queue alone passed 35GB).  Verdict is therefore BOUNDED, not          *)
(* exhaustive; the bound is deep enough to cover every defect class      *)
(* this document records.  To push further: a bigger disk, or            *)
(* -simulate for probabilistic depth beyond the BFS frontier.            *)
(*                                                                         *)
