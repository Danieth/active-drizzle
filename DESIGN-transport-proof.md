...
| O9 | Floor semantics implemented in the store: monotone deletion floor per entry, visibility interpretation, L3 garbage collection. (Replaces rev 2's tombstone-clearing clause, whose replace-semantics was the L2 counterexample.) | code |
...
| O12 | The deletion floor survives entry eviction and IndexedDB restore: a compact pk → floor map with retention dominating max in-flight + retries + restore-holding-a-GET. | code |
