...
- **Scope-blind gone(D) — the tombstone oracle (named, signed off
  2026-08-27).** A hard-delete tombstone stores no scope columns, so a
  gone(D) answered from it can never be tenant-checked. This is NOT T9's
  leak (T9 is epoch-scoped, to principals authorized before a
  revocation); an unrestricted gone(D) would answer arbitrary pk probes
  from principals who NEVER had access — existence, destruction, and the
  destroy token, across all tenants. Accepted residual, narrowed by
  construction: the validate handler answers hard-delete gone(D) ONLY
  through an UNSCOPED door (no URL scopes, no scopeBy — a door whose
  authenticated users could read every live row of the table anyway);
  SCOPED doors answer show's 404 and the client evicts via the legacy
  lane (correct, minus the floor's precision). Soft-deleted rows are
  re-scope-checked through the door's own relation before gone. What
  still leaks on an unscoped door: pk existence + destruction + D — to
  users already holding whole-table read. Recording scope columns in the
  tombstone is the named upgrade if a scoped door ever needs real
  gone(D).
...
| O5 | ✅ **DECIDED + server half DONE 2026-08-27 (WS3)** — Membership tag identity: the door-scoped commit-ordered COUNTER (the **theorem** grade), one row per door, bumped IN-COMMIT with every lifecycle write to the door's root model (`membership_tags` upsert on the write-log's executor — a sequence would survive rollback and break tag-equal ⇒ same-list). Conservative detection v1: create/destroy/undelete bump; value writes do not (v1's splice is always replace-all, so nothing consumes tag-equality as a skip yet; compiled scope-intersection is the named precision trim that must land before real ops do). SEPARATELY, the index-refetch 304 uses the pure STRUCTURE token — truncated SHA-256 over pk-set+order+count+cursor, facets excluded — the **probabilistic** grade, declared. Recorded in wire-identity §4. **Review-hardened 2026-08-27**: `insertAll` is now in-contract (bulk lifecycle=1 rows + ONE bump per call — a create with no bump violated the conservative-bump invariant); all doors of a table bump in ONE statement, and bulk paths (`updateAll`/`insertAll`) log via one multi-row INSERT + one bump; the write-path COST is now stated honestly (the counter row is a per-door serialization point held to commit — write-log.ts header carries the mitigation path); the index READ side omits the tag on an unmigrated DB instead of 500ing (the tag is an optional envelope field); splice REFUSES when no tag is producible (no read-after-list fallback, no frozen-0 counter on the wire) and the tag-read-BEFORE-list ordering remains a stated invariant with no deterministic pinning test yet (named test debt in WS3). **WS4 amendment 2026-08-27 (review-hardened)**: WS4's client began consuming tag-equality as a skip (reconnect catch-up), which the v1 clause "nothing consumes tag-equality as a skip yet" never licensed — a value write moving a row across a scoped/filtered list changes membership without a bump. License restored two-sidedly: (1) SCOPE-COLUMN value writes now bump in-commit (registerMembershipDoor carries the door's scope columns; membershipDoorsForValueWrite intersects them with the write's changed set — bounded cost, scope columns change rarely; the emitter fans an ids-only membershipHint door-wide so the OLD tenant's lists invalidate live), and (2) the client fires onTag on EVERY re-ack, tag-equal included — across a gap, tag-equality is never consumed as a skip. Residual, restated where consumed (ws-channels §6, docs/guide/channels.md): membership driven by arbitrary client-side index FILTERS still does not bump (the door cannot see them); such lists are corrected by the row's own CHANGE where it stays visible and heal on refetch/reconnect otherwise. scopeBy doors have no static column to register — same residual. The compiled scope-intersection trim remains the precision fix that must land before real splice ops do. | design ¶ → code |
| O7 | Information-flow model for T9, if external validation must cover the leak boundary. | design ¶ |
| O8 | ✅/⏳ **substantially DONE 2026-08-27 (WS6)** — specs/RowLane.tla + specs/DocLane.tla under TLC 2.19. DocLane: GREEN AND EXHAUSTED (1,118,481 distinct states, all invariants + MonotoneCursor/PerSeqReceipt/ConvergenceAfterQuiescence) — no-cursor-skip mechanized. RowLane (Rule M + floor + A2′ + epochs + three-phase pulls): TLC **found and killed a real M4 unsoundness** (the apply-time lastSeen ≥ W guard — proof amended, store patched, trace pinned as a test); post-fix BOUNDED verdict: no violation in 316.6M distinct states, complete BFS through depth 14 (pre-fix violation lived at depth 10); full exhaustion exceeds one machine's disk — results blocks in the specs state exactly what ran. The seeded property suite remains the runtime shadow. | mechanization |
...
| O10 | ✅ **server side DONE 2026-08-27 (WS3)** — A2′ implemented literally: the in-transaction write-log (`record_write_log`: per-commit token + declaration-order changed bitmap + lifecycle flag 0/1/2/3, written INSIDE runWritePhase / destroy's CAS / updateAll's RETURNING / counter-cache bumps — never afterCommit, so log-row-exists ⟺ commit-happened and gone(D) is a theorem, not a recovered guess), and the three-way `validate` procedure on every columnar door (`{status:'fresh',v} | {status:'gone',d} | {status:'stale',envelope}` — the envelope is buildColumnarEnvelope's bytes, A0). Clause (iii) is read from the row itself and never skipped at V==W; the gap rule (any missing token in (W,V] ⇒ slice) doubles as the retention/expiry rule — ONLY lifecycle=2 tombstones are exempt forever (creates are outside every interval since W ≥ 0, and a pruned undelete degrades to the slice; immortal create rows were pure unbounded growth + a pk-reuse collision trap). Pinned by `core/tests/integration/write-log.test.ts` + `react/tests/transport-forbidden-corruption.test.ts` (the two O8 forbidden-corruption targets on real PG through the real router). **Client half DONE 2026-08-27**: `react/src/validation-client.ts` `revalidateProjection` — ONE dispatch module (signal join ⇒ currency skip ⇒ unheld⇒fetch-never-validate ⇒ W=projFreshAt at issue ⇒ fresh⇒certify(P, V, the SAME issue-time W — feeding the store's M4 apply-time guard) / gone⇒destroy(D) / stale⇒mergeEnvelope / scope-miss 404⇒legacy remove()); generated doors embed the mask + projId as literals via the shared projIdFor and call the module (`.with().revalidate`); `shareMembershipData` is the structure-token guard on the generated index queries. Pinned by `react/tests/validation-client.test.ts` (incl. the M4 in-flight-replay drive through the module) and the real-PG end-to-end dispatch describe in `react/tests/transport-forbidden-corruption.test.ts`. **Review-hardened 2026-08-27**: (1) SOUNDNESS — fieldsRev reconciliation is now WIRED (lazily, memoized before the first bitmap read per table per process — no boot knob to forget; the previous "boot-time" claim had zero production callers, leaving column-reorder deploys able to misread bitmaps into wrong 304s); the fixture masks are no longer declaration-order prefixes (interleaved secretRate), so the clause-(i) probe is pinned in the DECLARATION numbering space. (2) AUTH — validate/splice run `only:`-scoped @before/@after hooks under their CRUD aliases (validate ≈ get, splice ≈ index — conservative union, pinned by the guarded-door test); hard-delete gone(D) is refused on scoped doors (see the §6 tombstone-oracle non-theorem); scoped-door validate/splice tenancy is pinned (scopeBy test). (3) REGISTRY — `registerLoggedModel` verifies the PHYSICAL lock column (lazily post-boot; declaration alone over-registered untracked models into permanent wrap overhead with zero log rows); an unlogged root's validate answers the slice and never touches the registry (was a 500 with registry internals for `lockingColumn=false` doors); codegen warns (W9) and the react generator skips the client transport symmetrically; the mask's ONE known codegen/runtime divergence (Attr property→column rename) now warns at router build naming the field. (4) CONTRACT — `insertAll` logs bulk creates; `deleteAll` refuses on logged models (teaching error naming `destroyAll`); undelete-ONLY intervals, the clause-(iii) record path (a door serving soft-deleted rows), W>V, destroy/updateAll rollback atomicity, and bulk soft-delete/restore lifecycle classification are all newly pinned. Residual (named): the codegen and runtime masks are still TWO computations sharing one rule + hash — the codegen-emitted server registry that makes it ONE is the follow-up; until then drift degrades to permanent conservative slices with a build/boot warning, never unsoundness. | code + design |
| O11 | Restate every "session guarantees" citation in the design docs as T6's scoped claims; drop or qualify Terry. | docs |
| O12 | ⏳ **store half DONE 2026-08-27 (WS1)** — The deletion floor survives entry eviction and IndexedDB restore: a compact pk → floor map with retention dominating max in-flight + retries + restore-holding-a-GET. Landed: floors authority map (never LRU-evicted), exportFloors/importFloors (max-join; reconciles existing entries, so import-after-merge heals itself), revision-keyed floorRetention (default: keep forever; finite retention is an explicit T2 trade — 𝒞w delay is unbounded — pinned by tests incl. keep-alive through merge/signal/certify and eviction re-seeding a pruned FloorRec from the entry's render copy). Open: the actual IndexedDB persistence wiring (restore path = importFloors before first merge; no store rewrite needed). **Server-side symmetry (WS3):** the write-log's retention mirrors floorRetention exactly — lifecycle=0 rows age out (default ~72h; expiry ⇒ the gap rule ⇒ conservative slice, self-healing), while lifecycle≠0 rows are EXEMPT FOREVER: the tombstone map is the server twin of the never-LRU-evicted floor authority map, one tiny row per destroyed lineage, because gone(D) has no other lawful source (T4). | code |
...
| O15 | ⏳ **contract pinned 2026-08-27 (WS3 v1)** — Splice application correctness: server-produced ops satisfy apply(list@from, ops) = list@to; tag identity does not imply it. v1's splice endpoint always answers `ops = [replace-all with list@to]`, under which the axiom holds trivially — and the property test (`transport-forbidden-corruption.test.ts`, applySplice against the door's real lists) pins the contract any REAL ops implementation must satisfy before it may ship. The obligation stays TCB-grade for that future implementation. **2026-08-27**: splice's `paramsHash` (the future ops key) is now pinned for distinctness and key-order invariance; splice refuses instead of serving a tagless (frozen-0) answer; the O15 property additionally holds through a scoped (scopeBy) door. | TCB |
| O16 | ✅ **DONE 2026-08-27 (WS4)** — Subscription epochs carried on every frame; RESET bumps the epoch; the client drops pre-epoch frames. Landed as the fixed 9-byte binary header (type/subId/epoch/bodyLen — work doc Appendix A as amended): the gateway stamps per-(connection,subscription) epochs at SOCKET-WRITE time (epochs never ride the bus) and bumps ONLY on RESET (emission-time re-check failure, REAUTH failure — a failed re-check always sends RESET, never a silent drop); the client (`react/src/channels.ts`) judges every CHANGE/SIGNAL on `peekHeader` ALONE — epoch < current(subId), or a retired/unknown subId, drops the frame BEFORE any msgpack/JSON parse touches the bytes, and RESET adopts the new epoch first so every old-generation frame is outlawed from that line on. Pinned three times over: unit (`react/tests/channels.test.ts` replay-drop), server (`controller/tests/channels/gateway.test.ts` revocation ⇒ RESET with bumped epoch, then silence), and the full-stack acceptance (`controller/tests/channels/acceptance.test.ts` scenario 4 — T9(ii) as a test: a CAPTURED pre-RESET CHANGE frame and a forged would-win-the-merge frame under the old epoch, injected at a real socket after a real revocation RESET, leave the store byte-untouched — the epoch filter, not the token floor and not delivery order, is the boundary). **Review-hardened 2026-08-27**: (1) the T9 bound now actually holds on BOTH lanes — flushIndexSub previously ran NO re-check (no RESET was ever produced for an index sub, so a revoked index subscriber streamed forever, contradicting the ticked claim); the revalidate TTL now re-runs the index dry-run at flush and failure RESETs (pinned: gateway hook-gated door test), and destroy frames on record subs are pass-gated too (expired pass ⇒ RESET, re-answered through the validate/tombstone fences — the §6 oracle triple never rides a stale pass). (2) A2 extended-to-frames is now true at the SOURCE: commit events carry a snapshot instance frozen at the write point, so the coalescing window can no longer pair post-save mutations with a committed token (the corruption the 304 lane would have certified permanently — pinned by the overlapping-writes drive). (3) The client epoch table is a MAX-JOIN at RESET and SUB_ACK (every monotone quantity is a max-join): a replayed/forged old-epoch RESET cannot regress the filter — the epoch table itself, not the retired-subId map, is now the enforced boundary (pinned by a subId-reuse drive). (4) Cross-tenant metadata (pk/token/op) is inside the leak fence: scoped doors per-pk dry-run every record-less index event before any SIGNAL, and the emitter strips unplaceable records from door-wide publishes — T9's analysis covers metadata to principals who NEVER had access, and now so does the implementation. | protocol |
